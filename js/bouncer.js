...
// 1. Ваш код, який «гальмує» blur для порожніх полів
document.addEventListener(
  'blur',
  function (e) {
    // Якщо елемент - інпут, і він порожній, зупинити подальші обробники
    if (e.target.matches('input, select, textarea') && !e.target.value.trim()) {
      e.stopImmediatePropagation();
    }
  },
  true
); // важливо: true = фаза capture

const contactFormBouncer = new Bouncer('.js-contact-form', bouncerConfig);
const contactUsFormBouncer = new Bouncer('.js-contact-us-form', bouncerConfig);
